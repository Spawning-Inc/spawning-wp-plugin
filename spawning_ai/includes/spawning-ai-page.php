<?php
   ...
   function spawning_ai_page()
   {
       ...
       $nonce = wp_create_nonce('kudurru_full_node_checkbox_action');
                    ...
                    <h2><span>𒋧Kudurru </span><a class="titleLink" target="_blank" href="https://kudurru.ai">↗</a></h2> 
                        ...
                        <?php if (get_option('spawning_kudurru_enabled') === 'on') : ?>
                            ...
                            <div class="checkbox-option">
                                <label>
                                    <input type="checkbox" name="full_node" id="fullNodeCheckbox" <?php checked(get_option('kudurru_full_node'), 'on'); ?> />
                                    Enroll as a full time listening node
                                    <span class="info-icon"
                                        data-tooltip="By checking this, you'll be sending more traffic information to the network. Not recommended for inexperienced users. Please thoroughly test your site after enabling to verify your host performance is capable of running this feature.">ⓘ</span>
                                </label>
                                <!-- Embed the nonce here -->
                                <input type="hidden" id="fullNodeCheckboxNonce" value="<?php echo $nonce; ?>" />
                            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    var checkbox = document.getElementById('fullNodeCheckbox');
    checkbox.addEventListener('change', function() {
        var nonce = document.getElementById('fullNodeCheckboxNonce').value;
        var xhr = new XMLHttpRequest();
        xhr.open('POST', ajaxurl, true);  // Ensure ajaxurl points to the WordPress AJAX handler (usually admin-ajax.php)
        ...
        xhr.send('action=handle_full_node_checkbox&full_node=' + (checkbox.checked ? 'on' : 'off') + '&fullNodeNonce=' + nonce);
    });
});
