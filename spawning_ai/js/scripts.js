...
var UIManager = {
    ...
    toggleCheckboxStates: function () {
        ...
        var oppositeCheckboxes = document.getElementsByClassName("wppd-ui-toggle-opposite");
    ...
    enableUpdateButton: function () {
        ...
        var checkboxes = document.querySelectorAll('.wppd-ui-toggle, .wppd-ui-toggle-opposite');
        // Then selects the button.
        var button = document.getElementById('ai-update-button');

        UIManager.lastSentState = Array.from(checkboxes).map(checkbox => checkbox.checked).join();
        ...
        checkboxes.forEach(function (checkbox) {
            checkbox.addEventListener('change', function () {
                ...
                if (UIManager.lastSentState !== '') {
                    var stateboxes = document.querySelectorAll('.wppd-ui-toggle, .wppd-ui-toggle-opposite');
                    button.disabled = UIManager.lastSentState === Array.from(stateboxes).map(checkbox => checkbox.checked).join();
                }
                else {
            ...
            $('#aiForm').on('submit', function (e) {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        // 'action' is a string that identifies which server-side code to execute.
                        // 'form' is the serialized form data.
                        action: 'handle_ai_form',
                        form: $(this).serialize()
                    },
                    ...
                    error: function (error) {
                        // The code in this function will execute if the AJAX request fails.
                        // The error information is available in the 'error' variable.
